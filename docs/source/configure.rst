...
Use the packaged script ``feed2toot-register-app`` to register the feed2toot app for your account. It uses the OAuth authorization code flow—no passwords are collected.::

    $ feed2toot-register-app
    Instance URL (e.g. https://social.example.org): https://framapiaf.org
    ...
    User credentials written to /home/me/feed2toot/creds/feed2toot_usercred.secret

Two files are created in ``./creds`` by default: ``feed2toot_clientcred.secret`` and ``feed2toot_usercred.secret``. See the :ref:`Use feed2toot-register-app` section for more options for ``feed2toot-register-app``.

Create Feed2toot configuration
------------------------------
After using feed2toot-register-app, you'll need the credentials in the feed2toot configuration.
...
In order to configure Feed2toot, you need to create a feed2toot.ini file (or any name you prefer, finishing with the extension .ini) with the following parameters::
    ...
    ; Here you need the two files created by feed2toot-register-app
    user_credentials=/etc/feed2toot/credentials/feed2toot_usercred.secret
    client_credentials=/etc/feed2toot/credentials/feed2toot_clientcred.secret
...
- user_credentials: a file with the user credentials, generated by the command feed2toot-register-app
- client_credentials: a file with the client credentials, generated by the command feed2toot-register-app
